...
async def synthesize_speech(text: str) -> Optional[bytes]:
    """Generate speech audio using edge-tts (Microsoft Edge, free, no API key)."""
    try:
        import edge_tts
...
async def api_test_tts():
    ...
    audio = await synthesize_speech("Jarvis online.")
    return {"valid": bool(audio), "error": None if audio else "edge-tts failed"}
...
async def api_settings_status():
    ...
    return {
        ...
        "env_keys_set": {
            ...
            "edge_tts_voice": env_dict.get("EDGE_TTS_VOICE", "en-GB-RyanNeural"),
